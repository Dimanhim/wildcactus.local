...
<div class="col-md-10">
	<div class="menu">
		...
		<?php 
			$persentSplit = $function->getPercentSource($orders, $order);
	        $countSplit = $function->countSourceOrders($orders, $order); 
        ?>
			<tr>
				<td><a href="<?= Yii::$app->urlManager->createUrl(['admin/orders', 'source' => $order, 'quan' => 'all']) ?>"><?= $order ?></a></td>
				<td><?= $countSplit ?></td>
				<td><?= $persentSplit ?>%</td>
			</tr>
