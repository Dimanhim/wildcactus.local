...
<div class="col-md-10">
	<div class="menu">
		...
		<?php 
			$persentSplit = $function->getPercentForm($orders, $order);
	        $countSplit = $function->countFormOrders($orders, $order); 
        ?>
			<tr>
				<td><a href="<?= Yii::$app->urlManager->createUrl(['admin/orders', 'form' => $order, 'quan' => 'all']) ?>"><?= $order ?></a></td>
				<td><?= $countSplit ?></td>
				<td><?= $persentSplit ?>%</td>
			</tr>
