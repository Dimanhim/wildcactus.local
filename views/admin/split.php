...
<div class="col-md-10">
	<div class="menu">
		...
		<?php 
			$persentSplit = $function->getPercentSplit($orders, $order);
	        $countSplit = $function->countSplitOrders($orders, $order); 
        ?>
			<tr>
				<td><a href="<?= Yii::$app->urlManager->createUrl(['admin/orders', 'split' => $order, 'quan' => 'all']) ?>"><?= $order ?></a></td>
				<td><?= $countSplit ?></td>
				<td><?= $persentSplit ?>%</td>
			</tr>
