...
<div class="col-md-10">
	<div class="menu">
		<h3>ID объявления / Тип / Позиция</h3>
		...
		<?php 
			$persentSplit = $function->getPercentContent($orders, $order);
	        $countSplit = $function->countContentOrders($orders, $order); 
        ?>
			<tr>
				<td><a href="<?= Yii::$app->urlManager->createUrl(['admin/orders', 'content' => $order, 'quan' => 'all']) ?>"><?= $order ?></a></td>
				<td><?= $countSplit ?></td>
				<td><?= $persentSplit ?>%</td>
			</tr>
